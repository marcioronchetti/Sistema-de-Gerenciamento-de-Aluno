...
if(isset($_POST["cod_disciplina"]) && !empty($_POST["cod_disciplina"])){
    ...
    $sql = "DELETE FROM disciplinas WHERE disciplinas.cod_disciplina = ?";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        // Definir parâmetros
        $param_id = trim($_POST["cod_disciplina"]);
    ...
    if(empty(trim($_GET["cod_disciplina"]))){
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="alert alert-danger fade in">
                            <input type="hidden" name="cod_disciplina" value="<?php echo trim($_GET["cod_disciplina"]); ?>"/>
                            <p>Têm certeza que deseja remover este dado?</p><br>
                            <p>
                                <input type="submit" value="SIM" class="btn btn-danger">
                                <a href="vizualizarDiscip.php" class="btn btn-default">Não</a>
                            </p>
                        </div>
                    </form>
                </div>
